...
import pandas as pd 
import os
for dirname, _, filenames in os.walk('../data/Respiratory_Sound_Database'):
...
mypath = "../data/Respiratory_Sound_Database/Respiratory_Sound_Database/audio_and_txt_files/"
...
p_diag = pd.read_csv("../data/Respiratory_Sound_Database/Respiratory_Sound_Database/patient_diagnosis.csv",header=None)
...
# for file_name, label in zip(filepaths, labels):
#     data = extract_features(file_name)
#     if data is not None:
#         features.append(data)
#         labels_clean.append(label)
#     else:
#         print("Skipping file due to extraction error:", file_name)
...
model.save('lung_cnn.h5')
print("Model saved as lung_cnn.h5")
# CONFUSION MATRIX
# matrix = confusion_matrix.astype(float) / confusion_matrix.sum(axis=1)[:, np.newaxis]
